...
def compute_centerline_and_control(
    ...
    empty_ctrl = {
        ...
        'cte_m': 0.0, 'heading_deg': 0.0,
        'mode': 'none', 'status': 'no centerline',
        'channel_width_px': 0.0, 'n_filtered': 0,
        'center_pts': [], 'path_pts': [],
    }
    ...
    # ── 4. 线性插值 → 稠密行覆盖，消除稀疏行导致的空交集 ──
    left_dense  = _interp_dict(left_dict)
    right_dense = _interp_dict(right_dict)
    ...
    ctrl = {
        'steering_deg': round(heading_deg, 2),
        'throttle_pct': round(max(0.0, (1.0 - 0.4 * abs(heading_deg) / 90.0)) * 100.0, 1),
        'cte_m': round(cte_m_val, 4),
        'heading_deg': round(heading_deg, 2),
        'mode': 'dual',
        'status': 'ok',
        'channel_width_px': round(channel_width_px, 1),
...
def _draw_compact_hud(
    ...
    """绘制简洁 HUD：左上角显示横向偏差，右上角显示状态（均用 PIL 渲染中文）。"""
    status = 'ok'

    # 左上第一行：横向偏差
    cte_col = (0, 255, 120) if abs(cte_m) < 0.2 else (0, 180, 255) if abs(cte_m) < 0.5 else (0, 80, 255)
    cte_dir = "偏右" if cte_m > 0 else "偏左" if cte_m < 0 else "居中"
    cte_txt = f"{cte_dir} {abs(cte_m):.2f}m"

    # 左上第二行：航向角
    hdg_txt = f"航向 {heading_deg:+.1f}deg"
    ...
    fs1, fs2 = 18, 14   # 第一行字号、第二行字号
    _, th1 = _cn_text_size(cte_txt, fs1)
    ...
    _put_cn_texts(canvas, [
        (cte_txt, (8, y1),                 fs1, cte_col,       2, (0, 0, 0)),
        (hdg_txt, (8, y2),                 fs2, (200, 220, 255), 2, (0, 0, 0)),
...
def main():
    ...
    ctrl_csv_file = None
    if out_dir:
        ctrl_csv_path = out_dir / 'control_signals.csv'
        ctrl_csv_file = open(str(ctrl_csv_path), 'w')
        ctrl_csv_file.write(
            'frame,steering_deg,throttle_pct,heading_deg,cte_m,'
            'channel_width_px,mode,status\n'
        )
    ...
    try:
        for img_path in img_paths:
            ...
            filtered_str = f"  [过滤倒影 {ctrl['n_filtered']}]" if ctrl['n_filtered'] > 0 else ""

            # 打印控制信号
            print(f"  {img_path.name}  边界px={n_bnd_px:5d}  "
                  f"mode={ctrl['mode']}  "
                  f"heading={ctrl['heading_deg']:+6.1f}°  "
                  f"steer={ctrl['steering_deg']:+5.1f}°  "
                  f"throttle={ctrl['throttle_pct']:.0f}%  "
                  f"CTE={ctrl['cte_m']:+.3f}m"
                  f"{filtered_str}")
            ...
            if ctrl_csv_file:
                ctrl_csv_file.write(
                    f"{img_path.name},"
                    f"{ctrl['steering_deg']:.2f},"
                    f"{ctrl['throttle_pct']:.1f},"
                    f"{ctrl['heading_deg']:.2f},"
                    f"{ctrl['cte_m']:.4f},"
                    f"{ctrl['channel_width_px']:.1f},"
                    f"{ctrl['mode']},"
                    f"{ctrl['status']}\n"
                )
    ...
    if ctrl_csv_path:
        print(f"  控制信号已导出: {ctrl_csv_path}")
    print(f"{'='*60}")
    print(f"\n[控制信号格式]")
    print(f"  steering_deg : 航向角偏差（度），+右偏 -左偏，0=正前方")
    print(f"  throttle_pct : 油门百分比 0~100（转弯越大越小）")
    print(f"  heading_deg  : 路径航向角（度），给飞控做航向参考")
    print(f"  cte_m        : 横向偏差（米），中线偏离航道中心的物理距离")
