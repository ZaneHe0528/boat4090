...
def infer_staged(
    ...
    post_h = min(int(input_height), int(oh))
    post_w = min(int(input_width), int(ow))
        ...
        with torch.no_grad():
            if post_stage_timers is None:
                logits = F.interpolate(
                    logits, size=(post_h, post_w), mode='bilinear', align_corners=False,
                )
...
def infer_staged_tensorrt(
    ...
    with t_post:
        if post_stage_timers is None:
            logits = F.interpolate(logits, size=(post_h, post_w), mode='bilinear', align_corners=False)
...
def run_benchmark(
    ...
    if detailed_timing and post_detail is not None and proc_detail is not None:
        print("  【细分计时】")
        print(f"    ③a 插值到后处理尺寸   {post_detail['post_resize'].mean_ms():>7.1f} ms")
